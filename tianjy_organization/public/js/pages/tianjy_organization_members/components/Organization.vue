<template>
	<div class="organization" v-loading="loading">
		...
		<ElTable :data="organizationList" border style="width: 100%" height="100%">
			<ElTableColumn prop="organization_doc.label" label="组织" />
			<ElTableColumn prop="default" label="默认组织" >
				<template #default="scope">
					<div>{{ scope.row.default===1?'是':'' }}</div>
				</template>
			</ElTableColumn>
			<ElTableColumn align="center" v-if="permissions.writePermission||permissions.deletePermission" prop="address" label="操作" width="340">
				<template #default="scope">
...
import {
	ElMessage, ElMessageBox, ElButton, ElTable, ElTableColumn, vLoading,
} from 'element-plus';
