<template>
	<Page>
		...
		<ElTabs v-model="activeName" class="organization-tabs">
			...
			<ElTabPane v-if="organization?.type_doc.no_workspace===0" class="tab-container" label="工作区" name="workspace">
				<Workspace v-if="organization" :organization="organization.name"></Workspace>
			</ElTabPane>
			<ElTabPane class="tab-container" label="成员" name="users">
				<Member v-if="organization" :organization="organization.name" :permissions="memberPermissions"></Member>
			</ElTabPane>
			<ElTabPane class="tab-container" label="继承" name="inherit">
				<Inherit v-if="organization" :organization="organization.name" :permissions="inheritPermissions"></Inherit>
			</ElTabPane>
			<ElTabPane class="tab-container" label="继承人员" name="inherit_members">
