...
class Page {
	set_document_title = true;
	buttons = {};
	fields_dict = {};
	views: Record<string, JQuery> = {};
	constructor(parent: HTMLElement, single_column?: boolean | 0 | 1) {
		this.parent = parent;
		this.single_column = single_column ? true : false;
		// frappe.ui.pages[frappe.get_route_str()] = this;
		this.wrapper = $(parent);
		const mainView = $(this.wrapper)
			.find('.page-content').find('.row.layout-main');
	...
	setup_sidebar_toggle() {
		let sidebar_toggle = this.wrapper.find('.tianjy-organization-form-detail-page-head ').find('.sidebar-toggle-btn');
	...
	set_indicator(label, color) {
		this.clear_indicator().removeClass('hide').html(`<span>${label}</span>`)
			.addClass(color);
	}
	...
	add_sidebar_item(label, action, insert_after, prepend) {
		...
		let li = $('<li>');
		let link = $('<a>').html(label).on('click', action)
			.appendTo(li);
	...
	add_view(name: string, html: string | JQuery) {
		let element = html;
...
interface Page {
	...
	btn_primary: JQuery
	btn_secondary: JQuery

	menu: JQuery
	menu_btn_group: JQuery

	actions: JQuery
	actions_btn_group: JQuery

	standard_actions: JQuery
	custom_actions: JQuery

	page_form: JQuery
	inner_toolbar: JQuery
	icon_group: JQuery
}
export default Page;
