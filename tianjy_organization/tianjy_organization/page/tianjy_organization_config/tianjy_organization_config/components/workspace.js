...
export default class Workspace extends frappe.views.Workspace {
	...
	build_sidebar_section(title, root_pages) {
		...
		$title.on('click', e => {
			...
			$(e.target).parent().find('.sidebar-item-container')
				.toggleClass('hidden');
		});
	...
	prepare_sidebar(items, child_container, item_container) {
		for (const item of items) { this.append_item(item, child_container); }
	...
	append_item(item, container) {
		...
		const isCurrentParent = descendant.some(page=>page.name=== this.get_page_to_show().name&&
		...
		if (this.organization_name === organization) { return; }
		$item_container.hide();
	}
	...
	get_page_to_show() {
		let default_page;
		const firstOrganizationPage = this.all_pages?.find(item=>item.organization === this.organization_name);
		const defaultOrganizationPage = this.all_pages?.find(item=>item.organization === this.organization_name&&item.default===1);
		...
		let page = this.current_page.name ||
			default_page.name;
	...
	save_page(page) {
		...
		return this.editor
			...
			.then(outputData => {
				...
				for (const item of outputData.blocks) {
					if (item.data.new) {
